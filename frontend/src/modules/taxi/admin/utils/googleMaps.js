...
// @react-google-maps/api's loader is a PROCESS-WIDE SINGLETON. Every
// useJsApiLoader call must pass identical options or it throws "Loader must not
// be called again with different options" and every map in the app white-
// screens. So there is exactly one id and one library list here, shared by
// admin, driver and rider. Do not split this.
...
// Kept as a name only. The singleton above means every caller must share one
// option set, so this is an alias -- it cannot load a smaller library list
// without breaking the whole app's maps.
export const useLiteGoogleMapsLoader = useAppGoogleMapsLoader;
